...
- Implement a Flask application that formats a query result as a dictionary.
...
dictionary to hold a pet's data.

So, update `app.py` to replace this JSON string:

```py
pet_json = '{"id": 1, "name" : "Fido", "species" : "Dog"}'
```

with a dictionary that looks like this:
...
outer single quotes since it is not a string, and (2) single quotes are used
instead of double quotes to define keys and values.
...
Notice the browser display JSON (double quotes) even though the dictionary uses
single quotes.
...
dictionary is a data structure that looks similar to a JSON string, yet it is
not a string. Flask will automatically transform a dictionary to JSON if one is
passed to the `make_response()` function. By returning JSON instead of HTML, a
client application can transform the server response into a wide variety of user
